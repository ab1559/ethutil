...
var nodeUrlMap = map[string]string{
	...
	nodeAvax:    "https://api.avax.network/ext/bc/C/rpc",
}
...
var nodeTxExplorerUrlMap = map[string]string{
	...
	nodeAvax:    "https://snowtrace.io/tx/",
}
...
var nodeApiUrlMap = map[string]string {
	...
	nodeAvax:  "https://api.snowtrace.io/api?module=contract&action=getsourcecode&address=%s",
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&globalOptNode, "node", "", "kovan", "mainnet | ropsten | kovan | rinkeby | goerli | sokol | bsc | heco | avax, the node type")
